...
class SendToServDataExportGui( DataExportGui ):
    """
    A subclass of the generic data export gui that creates custom layer viewers.
    """
    def createLayerViewer(self, opLane):
        return SendToServResultsViewer(self.parentApplet, opLane)


class SendToServResultsViewer(DataExportLayerViewerGui):
    
    def __init__(self, *args, **kwargs):
        super(SendToServResultsViewer, self).__init__(*args, **kwargs)
    ...
    def setupLayers(self):
        ...
        # see comment above
        for name, expected in zip(selection_names[0:3], ['Result', 'Thresholded Result', 'Labels']):
        ...
        if selection.startswith('Thresholded Result'):
